...
export default function ShareButtons({ url, title, description }: ShareButtonsProps) {
  ...
  const shareLinks = [
    ...
    },
    {
      ...
      color: "#4680C2",
      getUrl: () => `https://vk.com/share.php?url=${encodeURIComponent(url)}&title=${encodeURIComponent(title)}`,
    },
    {
      name: "Twitter",
      icon: (
  ...
  return (
    <div className="flex flex-col gap-4">
      <span className="text-white/50 text-sm">Поделиться:</span>
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/20 transition-all relative"
          title="Копировать ссылку"
        >
          {copied ? (
            <svg className="w-5 h-5 text-[#00FF9D]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <AnimatePresence>
            {copied && (
              <motion.span
                ...
                exit={{ opacity: 0 }}
                className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-xs text-[#00FF9D] whitespace-nowrap"
              >
                Скопировано!
              </motion.span>
            )}
